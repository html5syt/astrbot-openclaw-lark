...
async def _search_doc_wiki(event: Any, **kw: Any) -> str:
    client = get_lark_client()

    try:
        query = kw.get("query", "")
        filter_data = kw.get("filter")

        request_data: dict = {
            "query": query,
        }
        if kw.get("page_size"):
            request_data["page_size"] = kw["page_size"]
        if kw.get("page_token"):
            request_data["page_token"] = kw["page_token"]
...
SearchDocWikiTool: FunctionTool = make_tool(
